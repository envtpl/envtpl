...
class TestRender(unittest.TestCase):
    ...
    def test_die_on_missing(self):
        self.assertRaises(
            envtpl.Fatal, envtpl._render_string, "{{ FOO }}", {}, jinja2.StrictUndefined
        )
    ...
    def test_defaults(self):
        self.assertEqual(
            envtpl._render_string(
                '{{ FOO | default("abc") }}', {}, jinja2.StrictUndefined
            ),
            "abc",
        )
        self.assertEqual(
            envtpl._render_string(
                '{{ FOO | default("abc") }}', {"FOO": "def"}, jinja2.StrictUndefined
            ),
            "def",
        )
...
"""
        self.assertEqual(
            envtpl._render_string(string, {"FOO": 456}, jinja2.StrictUndefined),
            expected,
        )
...
"""
        self.assertEqual(
            envtpl._render_string(
                string, {"foo": "bar", "baz": "qux"}, jinja2.StrictUndefined
            ),
            expected,
        )
...
"""
        self.assertEqual(
            envtpl._render_string(
                string, {"X_foo": "bar", "baz": "X_qux"}, jinja2.StrictUndefined
            ),
            expected,
        )
...
"""
        ...
        self.assertEqual(
            envtpl._render_string(
                string, {"FOOS_JSON": foos_json}, jinja2.StrictUndefined
            ),
            expected,
        )
...
"""
        self.assertEqual(
            envtpl._render_string(
                string, {"FOO": '{"bar": "baz"}'}, jinja2.StrictUndefined
            ),
            expected,
        )
...
class TestFiles(unittest.TestCase):
    ...
    def test_bad_missing_output_filename(self):
        ...
        self.assertRaises(
            envtpl.Fatal, envtpl.process_file, ".tpl", None, {}, jinja2.Undefined, True
        )
...
+++"""
        envtpl.process_file(
            tpl_filename, None, {"FOO": "---", "BAR": "+++"}, False, True
        )
        with open(filename, "r") as f:
            self.assertEqual(f.read(), expected)
...
+++"""
    ...
    def test_no_delete(self):
        ...
        envtpl.process_file(
            tpl_filename, None, {"FOO": "---", "BAR": "+++"}, False, False
        )
        with open(filename, "r") as f:
    ...
    def test_die_on_missing(self):
        ...
        self.assertRaises(
            envtpl.Fatal, envtpl.process_file, tpl_filename, None, {}, True, False
        )
